...
star_insert_sql = """
    ...
    VALUES 
        (
            %(name)s, %(name_hd)s, %(name_hip)s,
            %(constellation)s,
            %(type)s, %(mass)s, %(radius)s, %(teff)s, %(luminosity)s, %(fe_h)s,
            %(age)s, %(appar_mag)s, %(distance)s, %(right_ascension)s, %(declination)s,
            %(num_planets)s, %(num_planets_hab_zone)s,
            %(hab_zone_min)s, %(hab_zone_max)s,
            %(hab_cat)s
        )
"""
...
def load_stars(engine, cursor):

    print('truncating star...')
    cursor.execute("TRUNCATE stars CASCADE")
    ...
    for j, row in df.iterrows():
        ...
        #print(values)
        cursor.execute(star_insert_sql, values)

    print('done inserting star data ...')

def load_planets(engine, cursor):

    # Drop everything from the planet table so we can safely reload it.
    print('truncating...')
    cursor.execute("TRUNCATE planets")
    ...
    for j, row in planets_df.iterrows():
        ...
        #print(values)
        cursor.execute(planet_insert_sql, values)
    
if __name__ == "__main__":

    connect_string = 'postgresql+psycopg2://'
    connect_string += 'jevans:99luftbaloons@localhost:5432/kepler'
    engine = sqlalchemy.create_engine(connect_string)

    conn = psycopg2.connect(database='kepler')
    cursor = conn.cursor()

    load_stars(engine, cursor)
    load_planets(engine, cursor)
